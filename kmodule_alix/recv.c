...
struct globals G;
extern struct mutex apoperationmutex;
...
//	printk(KERN_INFO "Valor de signal %d,",rxs->signal);
//	printk(KERN_INFO "BSSID %pM\n",common2->curbssid);
}
...
static int dump_add(struct sk_buff *skb,unsigned char *bssid,int signal,int caplen) {
    ...
    if( (ap_cur == NULL)  ) 
    {
        if( ! ( ap_cur = (struct AP_info *) kmalloc(
                         sizeof( struct AP_info ) , GFP_KERNEL) ) )
        {
...
//        ap_cur->tlast = time( NULL );
    ...
    ap_cur->tinit = jiffies;


    ap_cur->power_index = ( ap_cur->power_index + 1 ) % NB_PWR;
    ap_cur->power_lvl[ap_cur->power_index] = signal;
...
//            }
   ...
   if( skb->data[0] == 0x80 || skb->data[0] == 0x50 )
    {

        ap_cur->preamble = ( skb->data[34] & 0x20 ) >> 5;

        p = skb->data + 36;

        while( p < skb->data + caplen )
        {
            if( p + 2 + p[1] > skb->data + caplen )
                break;
...
int ath_rx_tasklet(struct ath_softc *sc, int flush, bool hp)
{
	...
	do {
        ...
        if( (skb->data[0] & 0x0C) == 0x04)
                goto out;
        
        /* if it's a LLC null packet, just forget it (may change in the future) */
        if(skb->len-4 > 28) {
                if(skb->data[24] == 0 && skb->data[25] == 0 && skb->data[26] == 0 && skb->data[27] == 0)
                goto out;
        }
        ...
        switch(skb->data[1] & 3) {
                case  0: memcpy( bssid, skb->data + 16, 6 ); break;	// Adhoc
                case  1: memcpy( bssid, skb->data +  4, 6 ); break;	// ToDS
                case  2: memcpy( bssid, skb->data + 10, 6 ); break;	// FromDS
                default: memcpy( bssid, skb->data +  4, 6 ); break;	// WDS    
        }
    /* only update power if packets comes from
     * the AP: either type == mgmt and SA != BSSID,
     * or FromDS == 1 and ToDS == 0 */        
        
        if( (((skb->data[1] & 3) == 0) && (memcmp(skb->data + 10, bssid, 6) == 0)) || ((skb->data[1] & 3) == 2)) {
                                    
                  dump_add(skb,bssid,rxs->signal,skb->len-4);              
...
out:		
		...
		ieee80211_rx(hw, skb); // <---- creo que aqui esta la movida: goyo
